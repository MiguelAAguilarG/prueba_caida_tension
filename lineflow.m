...
busprt = 0;
   for L = 1:nbr;
       if busprt == 0
       fprintf('   \n'), fprintf('%6g', n), fprintf('      %9f', P(n)*basemva)
       fprintf('%9f', Q(n)*basemva), fprintf('%9f\n', abs(S(n)*basemva))
       ...
       SLT = SLT + SL;
         ...
         if nl(L)==n | nr(L)==n
         fprintf('%12g', k),
         fprintf('%9f', real(Snk)), fprintf('%9f', imag(Snk))
         fprintf('%9f', abs(Snk)),
         fprintf('%9f', real(SL)),
             if nl(L) ==n & a(L) ~= 1
             fprintf('%9f', imag(SL)), fprintf('%9f\n', a(L))
             else, fprintf('%9f\n', imag(SL))
             end
         else, end
  end
...
SLT = SLT/2;
fprintf('   \n'), fprintf('    Total loss                         ')
fprintf('%9f', real(SLT)), fprintf('%9f\n', imag(SLT))
