...
fprintf('n\t'), fprintf('(W)Carg\t'), fprintf('%%V(flujos)\t'), fprintf('%%V(formula)\t'), fprintf('dif\n')
for nnn=1:length(carga)
...
plot(carga,V_caida_porcentaje(:,mmm),carga,caida_tension_calculada_simple(:,mmm),carga,V_caida_porcentaje(:,mmm)-caida_tension_calculada_simple(:,mmm))
xlabel('Carga (W)')
ylabel('%\DeltaV')
legend('%\DeltaV(flujos)', '%\DeltaV(formula)','%\DeltaV(flujos) - %\DeltaV(formula)')
str = strcat('fp: ', num2str(fp_carga(mmm)));
text(carga(length(carga)), V_caida_porcentaje(length(carga),mmm), str)
text(carga(length(carga)), caida_tension_calculada_simple(length(carga),mmm), str)
